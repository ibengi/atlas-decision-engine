...
def build_summary(cache_path, start_ts, end_open_ts, events_attempted,
    ...
    return {
        ...
        "outputs": {
            ...
            "card": os.path.basename(out_card),
        },
        "columns": COLUMNS,
    }
...
def write_data_card(s: dict, out_path: str) -> None:
    ...
    card = f"""# REPLAY DATA CARD — btc15m point-in-time replay, 2026-02-01 → 2026-02-28

Status: **QA3b step 1 of 2 — ONE MONTH ONLY.** The six-month run is a separate
follow-up; this file documents the February dataset. Generated by
`research/replay_btc.py` and committed as the record of this run.

## Headline numbers

| Quantity | Value |
|---|---|
| Events attempted (15-min windows, open in Feb) | {c['events_attempted']} |
...
- **Fallback:** if the candle at `W` is absent (data gap), the strike falls
  ...
  to exclude them (they can). Zero gaps in February ⇒ 0 such rows here.
...
- The three February exclusions are the first event's t=15/t=10/t=8 decision
  ...
  interpolated. A six-month run has the same three boundary exclusions.
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--card", default="research/data/REPLAY-DATA-CARD.md")

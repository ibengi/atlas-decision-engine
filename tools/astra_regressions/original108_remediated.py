...
def run(name, fn, seed=True):
    ...
    try:
        f = Fixture(seed)
        fn(f)
        row = {'id':name, 'status':'PASS', 'detail':json.loads(json.dumps(DETAIL, default=str))}
    except AssertionError as e:
        row = {'id':name, 'status':'FAIL', 'reason':str(e), 'detail':json.loads(json.dumps(DETAIL, default=str))}
...
if not OLD:
    def journal_case(f, kind):
        ...
        if kind=='return_valid':
            ...
            complete_verified_recovery(f.led.path, f.led.identity, f.led.authority,
                checkpoint(f.led.path, f.led.identity).digest, 'return-valid-evidence')
            f.reload()
            DETAIL.update(f.snapshot())
    ...
    def rebase_variant(f,k):
        ...
        if k=='intent': f.orders._record_intent('KXBTC15M-AUDIT','cid',1,50,side='yes')
...
"""
    ...
    def transport(f):
        ...
        from state_authority import checkpoint
        from authority_double import CheckpointStore
        with tempfile.TemporaryDirectory(prefix='atlas-demo-control-') as control_root, patch.object(CFG,'DATA_DIR',control_root):
            ident = account_identity('kalshi','demo',CFG.BROKER_ACCOUNT_ID)
            c.continuity_authority = CheckpointStore(checkpoint(_p('transport_intents.json'), ident))
    ...
    def historic_ledger(f):
        for value in (-.3,-.1839):
            f.trade(value,observe=False,settled_at='2026-09-01T00:00:00Z')

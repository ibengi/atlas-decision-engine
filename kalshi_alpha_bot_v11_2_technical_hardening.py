...
ENGINE_VERSION = "v11.2-technical-hardening-2026-07-28"
...
class KalshiClient:
    ...
    def cancel_order(self, order_id: str) -> dict:
        """Annule un ordre cree via Order V2.

        Kalshi conserve GET /portfolio/orders/{id} pour la lecture, mais
        l'annulation des ordres V2 passe par /portfolio/events/orders/{id}.
        """
        try:
            r = self._req("DELETE", f"{self.ORDERS_V2_PATH}/{order_id}")
            log_api.info(f"[CANCEL_V2_SUCCESS] kalshi_order_id={order_id} "
            ...
            try:
                try:
                    ...
                    filled = min(meta["count"], sum(
                        pick_int(f, "count", "quantity", default=0) for f in fills))
